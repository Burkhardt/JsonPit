...
- `dotnet test JsonPit/JsonPit.Tests/JsonPit.Tests.csproj --nologo -v minimal` first surfaced one transient failure in `RAIkeepConcurrencyRegressionTests.SaveInterleavedWithAdds_SubsequentSavePersistsEveryAcceptedItem` with `IndexOutOfRangeException` from `Pit.GetRawPersistenceModel()`.
- `dotnet test JsonPit/JsonPit.Tests/JsonPit.Tests.csproj --filter FullyQualifiedName~SaveInterleavedWithAdds_SubsequentSavePersistsEveryAcceptedItem --nologo -v minimal` passed on immediate isolated rerun: `1` succeeded, `0` failed.
- `dotnet test JsonPit/JsonPit.Tests/JsonPit.Tests.csproj --nologo -v minimal` then passed on immediate rerun: `101` succeeded, `1` skipped, `0` failed.
